...
exports.item=function(req,res){
	...
	var smy_id=req.query.smy_id;
	var ctx={pa_id:person.id};

	step(
		function(){
			if(item_id>0){
				monthsmy_service.find_msitem(req.models,item_id,this);
			}else{
				monthsmy_service.create_msitem(req.models,smy_id,this);
			}
		},
		function(err,item){
			ctx.item=item;
...
exports.save_item=function(req,res){
	...
	if(pg_item.monthchk_id){
		...
		step(
			...
			function(err,monthsmy){
				pg_item.monthchk_id=monthsmy.id;
...
exports.delete_item=function(req,res){
	...
	monthsmy_service.delete_item(req.models,item_id,function(err){	
		if(err) console.info(err);
		res.send(true);
	})
}
...
exports.submit=function(req,res){
	...
	step(
		...
		function(err,items){
			ctx.msg=monthsmy_service.check_submit(items);
			if(ctx.msg.flag){
